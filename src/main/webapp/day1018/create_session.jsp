...
<div id="wrap">
<c:choose>
<c:when test="${ param.session eq 'create' }">
<% session.setAttribute("user_id","kim" ); %>
</c:when>
<c:when test="${ param.session eq 'remove' }">
<% session.removeAttribute("user_id");
   session.invalidate();
 %>
</c:when>
</c:choose>
	<a href="create_session.jsp?session=create">세션 생성</a>
	<a href="create_session.jsp?session=remove">세션 삭제</a>
	<a href="send_redirect.jsp">이동</a>
</div>
</body>
</html>